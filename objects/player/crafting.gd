...
var _craft_timer := PollTimer.new(0.5)
...
func _physics_process(delta: float) -> void:
	...
	_craft_timer.update(delta)
	
	# update item craft progress
	if crafted_item_prefab != null:
		if not _craft_timer.is_active:
			finish_item_craft()
...
func get_crafting_progress_ratio() -> float:
	return 1.0 - _craft_timer.get_progress_ratio()
...
func finish_item_craft():
	...
	Global.get_game().stamina_points -= \
			crafted_item_cost if not OS.is_debug_build() else 0
	crafted_item_prefab = null
...
func is_player_on_floor() -> bool:
	...
	# check also the the tile below the tile that the player's center is
	# occupied in is a floor tile. this is so that the bridge item can't place
	# the bridge inside the ground when you are very close to the ground, but
	# not actually on it.
	var _tilemap: TileMapLayer = Global.get_game().get_node("Map")
	var player_local_pos := _tilemap.to_local(player.global_position)
	var player_tilemap_pos = _tilemap.local_to_map(player_local_pos)
