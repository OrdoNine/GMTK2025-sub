...
func _handle_state_transitions():
	match current_state:
		PlayerState.FREEMOVE:
			...
			if _can_be_stunned and not Global.is_timer_active(Global.TimerType.STUN) and not Global.is_timer_active(Global.TimerType.INVINCIBILITY):
		...
		PlayerState.CRAFTING:
			...
			if _can_be_stunned and not Global.is_timer_active(Global.TimerType.STUN) and not Global.is_timer_active(Global.TimerType.INVINCIBILITY):
				...
				current_state = PlayerState.STUNNED
		PlayerState.STUNNED:
			if not Global.is_timer_active(Global.TimerType.STUN) and current_state == PlayerState.STUNNED:
