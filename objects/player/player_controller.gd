...
func _physics_process(delta: float) -> void:
	...
	if _deadly_area_count > 0 and _iframe_timer <= 0.0:
		take_damage()
...
func _process(_delta: float) -> void:
	...
	if is_stunned:
		...
		if item_craft_progress != null:
			var t: float = 1.0 - (
					item_craft_progress.time_remaining /
					item_craft_progress.wait_length
			)
...
func update_movement(delta: float) -> void:
	...
	# jump button is released, stop the jump and dampen the y velocity. makes
	# it easier to control the height of the jumps
	...
	if Input.is_action_pressed("player_jump") and not is_on_ceiling():
		if is_jumping:
			...
			var dt := delta / jump_length
			_jump_remaining = move_toward(_jump_remaining, 0.0, dt)
	else:
		if is_jumping:
	...
	match current_state:
		...
		PlayerState.WALLSLIDE:
			...
			# wall sliding
				...
				velocity.y = min(
					velocity.y,
					get_gravity().y * delta * wall_slide_speed,
				)
